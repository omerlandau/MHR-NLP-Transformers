...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, gamma_conf=None, batch_num=None):
        ...
        if gamma_conf is not None and (batch_num<0.6):
            ...
            loss += gamma_conf*(batch_num+0.5)*l_conf_enc + l_growth_enc*gamma_conf*(batch_num+0.3) + l_growth_dec_e*gamma_conf*(batch_num +0.3) \
                    + l_conf_dec_e*gamma_conf*(batch_num +0.3) + l_conf_dec*gamma_conf*(batch_num +0.3) +  l_growth_dec*gamma_conf*(batch_num +0.3)
