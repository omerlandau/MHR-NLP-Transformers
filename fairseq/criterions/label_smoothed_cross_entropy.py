    ...
    def forward(self, model, sample, reduce=True, gamma_conf=None, batch_num=None):
        ...
        if(batch_num<0.9999):
            ...
            for i in range(len(model.encoder.layers)):
                ...
                l_alpha_enc +=  3*(torch.norm(model.encoder.layers[i].self_attn.alphas, p='nuc').detach() + 0.1 - torch.norm(model.encoder.layers[i].self_attn.alphas, p='nuc'))

            for i in range(len(model.decoder.layers)):
                l_alpha_dec += 1.5*(torch.norm(model.decoder.layers[i].self_attn.alphas, p='nuc').detach() + 0.1 - torch.norm(model.decoder.layers[i].self_attn.alphas, p='nuc'))
                l_alpha_dec_e += 2*(torch.norm(model.decoder.layers[i].encoder_attn.alphas, p='nuc').detach() + 0.1 - torch.norm(model.decoder.layers[i].encoder_attn.alphas, p='nuc'))
        ...
        print(loss.data)
        loss += gamma_conf*(l_alpha_enc + l_alpha_dec_e + l_alpha_dec)
